...
void set_problem(struct Parameters* params, IndividualProblem conditions, double le) {
    params->g = 1.4;
    /* границы областей расчета */
    params->a = conditions.x_start;
    params->b = conditions.x_end;
    ...
    pvr.r = conditions.rL;
    params->params.push_back(pvr);
    params->l.push_back(-le/2); // Первая граница областей

    /* облаcть два */
    pvr.p = conditions.pExp;
    conditions.vExp = (-1) * sqrt(2 * conditions.e / (conditions.rExp * le));
    pvr.v = conditions.vExp;
    pvr.r = conditions.rExp;
    params->params.push_back(pvr);
    params->l.push_back(conditions.x_mid); // Вторая граница областей

    /* облаcть три */
    pvr.p = conditions.pExp;
    conditions.vExp = sqrt(2 * conditions.e / (conditions.rExp * le));
    pvr.v = conditions.vExp;
    pvr.r = conditions.rExp;
    params->params.push_back(pvr);
    params->l.push_back(le/2); // Третья граница областей
    ...
    /* ПАРАМЕТРЫ РАСЧЕТА*/
    params->cells_number = 1000;
    params->stop_time = conditions.time;
    params->CFL = 0.2;
    params->S_type = 1;
    params->fo = 50; // частота вывода файлов, выводится каждый 50-й шаг
    params->q = 0; //искусственная вязкость
    params->bound = conditions.bound; // тип границы 0 - жесткая стенка, 1 - нет жесткой стенки
}
...
int main() {
    ...
    // for (double le = 0.5; le < 45.0; le += 0.2) 
    {
        double le = 15.0;
        ...
        // Нужно для вычисления области поражения
        double best_x = 0;
        ...
        IndividualProblem conditions;
        set_problem(&params, conditions, le);
        ...
        std::cout << "le = " << le << ", best_x = " << best_x << std::endl;
        le_values.push_back(le);
        best_x_values.push_back(best_x);

    }

    // Выведем значения le и best_x в файл result.txt
    // std::ofstream fout;
    // fout.open("result.csv");
    // fout << "le" << ";" << "best_x" << std::endl;
    // for(int i = 0; i < le_values.size(); i++){
    //     fout << le_values[i] << ";" << best_x_values[i] << std::endl;
    // }
    // fout.close();
