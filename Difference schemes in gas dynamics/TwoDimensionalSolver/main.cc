...
void read_parameters(struct Parameters* params) {
    ...
    params->a = 0.0;
    params->l.push_back(params->a);
    params->l.push_back(0.1);
    params->l.push_back(0.9);
    params->b = 1.0;
    params->l.push_back(params->b);

    /* ПАРАМЕТРЫ РАСЧЕТА*/
    params->cells_number = 1000;
    params->stop_time = 0.038;
    params->CFL = 0.2;
    params->S_type = 3;
    params->fo = 50;
    params->q = 1;
...
/* ГРАНИЧНЫЕ УСЛОВИЯ */
void Boundary(double p, double v, double r, double& pb, double& vb, double& rb, int b_type){
    /* стенка */
    if (b_type == 0){
        pb = p;
        vb = -v;
...
int main() {
    ...
    while (time < params.stop_time){
        ...
        if (S_type == 2 || S_type == 3){
            ...
            for(int i = 0; i < N; ++i){
                if(i == 0){
                    Boundary(m[0], imp[0], e[0], mb, impb, eb , 0);
                    ...
                    Boundary(m[N - 1], imp[N - 1], e[N - 1], mb, impb, eb , 0);
        ...
        for(int i = 0; i < N; ++i){
            ...
            if (S_type == 1){
                ...
                if (i != 0){
                    ...
                    Boundary(m_kolgan_r[0], imp_kolgan_r[0], e_kolgan_r[0], mb, impb, eb , 0);
                    ml = mb;
                    impl = impb;
                    el = eb;
                    mr = m_kolgan_l[i];
            ...
            if (S_type == 1){
                ...
                if (i != N - 1){
                    ...
                    impl = imp_kolgan_r[i];
                    el = e_kolgan_r[i];
                    Boundary(m_kolgan_l[N - 1], imp_kolgan_l[N - 1], e_kolgan_l[N - 1], mb, impb, eb , 0);                   
                    mr = mb;
                    impr = impb;
                    er = eb;
                }
